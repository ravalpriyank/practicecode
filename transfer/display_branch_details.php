...
$order_columns = ["sno", "name", "code", "minnum", "maxnum", "city", "phone", "address", "branchwithcode"];
$order_column_name = $order_columns[$order_column_index];
$order_dir = isset($_POST['order'][0]['dir']) ? $_POST['order'][0]['dir'] : "asc";

if (!empty($search)) {
    $search_condition = "WHERE `sno` LIKE '%$search%' OR `name` LIKE '%$search%' OR `code` LIKE '%$search%' OR `minnum` LIKE '%$search%' OR `maxnum` LIKE '%$search%' OR `city` LIKE '%$search%' OR `phone` LIKE '%$search%' OR `address` LIKE '%$search%' OR `branchwithcode` LIKE '%$search%'";
} else {
    $search_condition = "";
}

$query = "SELECT * FROM `branchlog` $search_condition LIMIT $start, $length";
$run_query = mysqli_query($conn2, $query);

$total_query = "SELECT COUNT(*) AS 'total' FROM `branchlog` $search_condition";
